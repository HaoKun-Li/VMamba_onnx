...
def main(config, args):
    ...
    if args.inference_time_pytorch:
        image_tensor = transform(Image.open("/mnt/datasets/imagenet/val/n15075141/ILSVRC2012_val_00045266.JPEG")).unsqueeze(0).cuda()
    ...
    if args.convert_to_onnx:
        image_tensor = transform(Image.open("/mnt/datasets/imagenet/val/n15075141/ILSVRC2012_val_00045266.JPEG")).unsqueeze(0)
        image_tensor = image_tensor.repeat(args.batch_size, 1, 1, 1)
    ...
    if args.inference_time_onnx:
        ...
        print("begin laod model!")
        
        # 加载ONNX模型
        onnx_model = onnx.load(args.onnx_path)
        
        print("finish laod model!")

        # 验证模型
        onnx.checker.check_model(onnx_model)

        print("begin creat InferenceSession!")
        ...
        image_tensor = transform(Image.open("/mnt/datasets/imagenet/val/n15075141/ILSVRC2012_val_00045266.JPEG")).unsqueeze(0).cpu()
