...
try:
    ...
    from .csms6s_hu import CrossScan, CrossMerge
    from .csms6s_hu import CrossScan_Ab_1direction, CrossMerge_Ab_1direction, CrossScan_Ab_2direction, CrossMerge_Ab_2direction
    from .csms6s_hu import SelectiveScanMamba, SelectiveScanCore, SelectiveScanOflex
    from .csms6s_hu import flops_selective_scan_fn, flops_selective_scan_ref, selective_scan_flop_jit
except:
    from csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1, getCSM
    from csm_triton import CrossScanTritonF, CrossMergeTritonF, CrossScanTriton1b1F
    from csms6s_hu import CrossScan, CrossMerge
...
def selective_scan_easy(us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=True, return_last_state=False, chunksize=64):
    """
    ...
    # if len(Bs.shape) == 3:
    if len(map(int,Bs.shape)) == 3:
        Bs = Bs.unsqueeze(1)
    # if len(Cs.shape) == 3:
    if len(map(int,Cs.shape)) == 3:
        Cs = Cs.unsqueeze(1)
    # B, G, N, L = Bs.shape
...
def selective_scan_ref(
        ...
        **kwargs
        ...
        deltaB_u = F.conv1d(delta.contiguous().view(1,Batch*KCdim*L, 1), B.view(int(Batch*KCdim*L*N), 1, 1), groups=Batch*KCdim*L)
        ...
        if True:
            ...
            for i in range(L):
                ...
                y = F.conv1d(x.view(1,Batch*KCdim*N,1), C[:, :, :, i].contiguous().view(int(Batch*KCdim), int(N), 1), groups=Batch*KCdim).view(Batch, KCdim)
...
class SS2Dv0:
    ...
    def forwardv0(self, x: torch.Tensor, SelectiveScan = SelectiveScanMamba, seq=False, force_fp32=True, **kwargs):
        ...
        D, N = map(int,self.A_logs.shape)
        K, D, R = map(int,self.dt_projs_weight.shape)
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
            ...
            ### modify by lihaokun
            return SelectiveScan(u, delta, A, B, C, D, delta_bias, delta_softplus, ssoflex)
...
class SS2Dv3:
    ...
    def forwardxv(self, x: torch.Tensor, **kwargs):
        ...
        B, (H, W) = map(int,x.shape[0]), (map(int,x.shape[2:4]) if self.channel_first else map(int,x.shape[1:3]))
        L = H * W
        dt_projs_weight = self.dt_projs_weight
        A_logs = self.A_logs
        dt_projs_bias = self.dt_projs_bias
