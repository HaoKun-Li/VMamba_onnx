...
class CrossScan(torch.autograd.Function):
    ...
    def backward(ctx, ys: torch.Tensor):
        # out: (b, k, d, l)
        # B, C, H, W = ctx.shape
        B, C, H, W = map(int,ctx.shape)
        L = H * W
        ys = ys[:, 0:2] + ys[:, 2:4].flip(dims=[-1]).view(B, 2, -1, L)  # todo 计算B L W,H 并替换
...
class CrossMerge(torch.autograd.Function):
    ...
    def backward(ctx, x: torch.Tensor):
        ...
        # H, W = ctx.shape
        # B, C, L = x.shape
        H, W = map(int,ctx.shape)
        B, C, L = map(int,x.shape)
