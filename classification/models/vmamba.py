...
def CrossScan_onnx_no_flip(x: torch.Tensor):
    # B, C, H, W = x.shape
    B, C, H, W = map(int, x.shape)
...
def selective_scan_easy(us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=True, return_last_state=False, chunksize=16):
    ...
    def selective_scan_chunk(us, dts, As, Bs, Cs, hprefix):
        ...
        ### As conv ts
        # Ats_temp = (F.conv1d(As.contiguous().view(1, G*D, N), ts.permute(2, 3, 0, 1).contiguous().view(-1, 1, 1), groups=G*D))
        # Ats_temp = Ats_temp.exp().view(G, D, chunksize, B, N).permute(2, 3, 0, 1, 4).contiguous()
        
        # ### calculate Ats without einsum, modify by lihaokun at 20240710
        ### ts conv As
        Ats = F.conv1d(ts.contiguous().view(chunk_L*B, G*D, 1), As.contiguous().view(-1, 1, 1), groups=G*D)

        # ### debug
        # Ats_nan = torch.isnan(Ats).sum().item()

        Ats = Ats.exp().view(chunk_L, B, G, D, N).contiguous()

        # ### debug
        # Ats_exp_nan = torch.isnan(Ats).sum().item()

        # # original code
        # scale = Ats[-1].detach()

        # modify by lihaokun at 20240711
        # sometimes scale will be zero, so we add a minimal value to avoid divide zero
        # scale = Ats[-1].detach() + torch.finfo(torch.float32).tiny
        scale = Ats[-1].detach() + 1e-30

        rAts = Ats / scale
        duts = dts * us

        # ### debug
        # rAts_nan = torch.isnan(rAts).sum().item()

        # num_zeros_scale = torch.eq(scale, 0).sum().item()
        # print("scale has ", num_zeros_scale, " 0")  

        # if rAts_nan > 0:
        #     print("rAts_nan:{}".format(rAts_nan))
        #     nan_indices = torch.nonzero(torch.isnan(rAts))
        #     print("nan_indices:{}".format(nan_indices))
        #     print(Ats.size())
        #     print(scale.size())

        #     for i in range(nan_indices.size(0)):
        #         print(Ats[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])
        #         print(scale.view(1, B, G, D, N).repeat(chunk_L, 1, 1, 1, 1)[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])


        # ### debug
        # duts_nan = torch.isnan(duts).sum().item()
        ...
        # ### debug
        # temp_1 = torch.isnan(dtBus / (rAts + 1e-30)).sum().item()
        # temp_2 = torch.isnan((dtBus / (rAts + torch.finfo(torch.float32).tiny)).cumsum(dim=0)).sum().item()
        # print("temp_1:{} temp_2:{}".format(temp_1, temp_2))
        # hs_tmp_nan = torch.isnan(hs_tmp).sum().item()

        # if hs_tmp_nan > 0:
        #     print("hs_tmp_nan:{}".format(hs_tmp_nan))
        #     nan_indices = torch.nonzero(torch.isnan(hs_tmp))
        #     print("nan_indices:{}".format(nan_indices))
        #     print(rAts.size())
        #     print((dtBus / (rAts + 1e-30)).cumsum(dim=0).size())

        #     for i in range(nan_indices.size(0)):
        #         print(rAts[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])
        #         print((dtBus / (rAts + 1e-30)).cumsum(dim=0)[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])
        #         print((dtBus / (rAts + 1e-30))[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])
        #         print(dtBus[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])
        #         print((rAts + 1e-30)[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])
        #         print(rAts[nan_indices[i][0], nan_indices[i][1], nan_indices[i][2], nan_indices[i][3], nan_indices[i][4]])

        # ### debug
        # hs_nan = torch.isnan(hs).sum().item()

        # # original code
        # ys = torch.einsum("lbgn,lbgdn->lbgd", Cs, hs) 

        ### calculate Ats without einsum, modify by lihaokun at 20240710
        ys = F.conv1d(Cs.contiguous().view(1, chunk_L*B*G*N, 1), hs.contiguous().view(chunk_L*B*G*D, N, 1), groups=chunk_L*B*G)
        ys = ys.view(chunk_L, B, G, D).contiguous()

        # ### debug
        # ys_nan = torch.isnan(ys).sum().item()

        # print("{} {} {} {} {} {} {} {}".format(Ats_nan, Ats_exp_nan, rAts_nan, duts_nan, dtBus_nan, hs_tmp_nan, hs_nan, ys_nan))
    ...
    # As = As.view(G, -1, N).double()
    # Bs = Bs.permute(3, 0, 1, 2).double()
    # Cs = Cs.permute(3, 0, 1, 2).double()
    # Ds = Ds.view(G, -1).double() if has_D else None
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        if initialize in ["v0"]:   # vmambav2_tiny_2024 use this 
            # dt proj ============================
            self.dt_projs = [
                self.dt_init(dt_rank, d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor)
                for _ in range(k_group)
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
            ...
            # selective_scan_dict = {}
            # selective_scan_dict["u"] = u.cpu()
            # selective_scan_dict["delta"] = delta.cpu()
            # selective_scan_dict["A"] = A.cpu()
            # selective_scan_dict["B"] = B.cpu()
            # selective_scan_dict["C"] = C.cpu()
            # selective_scan_dict["D"] = D.cpu()
            # selective_scan_dict["delta_bias"] = delta_bias.cpu()
            # torch.save(selective_scan_dict, 'selective_scan_dict.pt')
            ...
            # relative_error = difference/result
        ...
        if cascade2d:   # False in vmambav2_tiny_224
            def scan_rowcol(
                x: torch.Tensor, 
        ...
        y = y.view(B, -1, H, W) # onnx is ok after here
        if not channel_first: # channel_first is true
            y = y.view(B, -1, H * W).transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1) # (B, L, C)
        y = out_norm(y) 

        return (y.to(x.dtype) if to_dtype else y)

    def forwardv2(self, x: torch.Tensor, **kwargs):  # vmambav2_tiny_224 use this
        x = self.in_proj(x)
        ...
        if self.with_dconv:
            x = self.conv2d(x) # (b, d, h, w)
        x = self.act(x) # onnx is ok before here
        y = self.forward_core(x)
        y = self.out_act(y) # identity in vmambav2_tiny_224
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y)) # onnx is ok after here
        return out
...
class SS2D(nn.Module, mamba_init, SS2Dv0, SS2Dv2, SS2Dv3):
    def __init__(
        ...
        **kwargs,
        ...
        # vmambav2_tiny_224 forward_type is v05_noz
...
class VSSM(nn.Module):
    def __init__(
        ...
        patchembed_version: str = "v1", # "v1", "v2"   # vmambav2_tiny_224 use this
        ...
        **kwargs,
        ...
        super().__init__()
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"]) # vmambav2_tiny_223 use LN2D
        ...
        _make_patch_embed = dict(
            ...
            v2=self._make_patch_embed_v2,  # vmambav2_tiny_224 use this 
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.pos_embed is not None: # vmambav2_tiny_224中为None
